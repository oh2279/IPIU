...
class VGG(nn.Module):
    def __init__(
        ...
        super().__init__()
        self.features, out_c = features
        #print(out_c)
        self.avgpool = nn.AdaptiveAvgPool2d((7, 7))
        self.classifier = nn.Sequential(
            nn.Linear(out_c*7*7, 4096),
            nn.ReLU(True),
...
def _vgg(cfg: str, batch_norm: bool, weights=None, progress: bool=True, **kwargs: Any) -> VGG:
...
def vgg5(*args, **kwargs):
    ...
    return _vgg('5', True, **kwargs)

def vgg9(*args, **kwargs):

    return _vgg('9', True, **kwargs)
